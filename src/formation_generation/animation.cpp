...
void PlotPose1(std::shared_ptr<formation_planner::PlannerConfig> config, int robot_index, int time_step, Trajectory_temp traj, std::vector<double> robot_type) {
...
void PlotPose(std::shared_ptr<formation_planner::PlannerConfig> config, int robot_index, int time_step, vector<vector<vector<double>>> plan, vector<double> robot_type) {
    const std::string robot_name = "Footprint_" + std::to_string(robot_index);
    // visualization::Delete(i, robot_name);
    // visualization::Trigger();
