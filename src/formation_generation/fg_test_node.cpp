...
#include <formation_generation/ecb_traj_optimization.hpp>
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "fg_test_node");
  ...
  ros::Publisher path_pub = nh.advertise<nav_msgs::Path>("/fg_test_path", 1, false);
  ros::Publisher path_pub_diff1 = nh.advertise<nav_msgs::Path>("/fg_test_path_diff1", 1, false);
  ros::Publisher path_pub_diff2 = nh.advertise<nav_msgs::Path>("/fg_test_path_diff2", 1, false);
  ros::Publisher path_pub_car1 = nh.advertise<nav_msgs::Path>("/fg_test_path_car1", 1, false);

  interactive_markers::InteractiveMarkerServer server_("/fg_obstacle");
  // vec_Vec2f obs;
  ...
  while(ros::ok()) {
    ...
    std::vector<formation_planner::Constraints> constraint_set;
    ...
    for (int robot_ind = 0; robot_ind < initTrajPlanner_obj->initTraj.size(); robot_ind++) {
      ...
      for(size_t i = 0; i < 5 * (initTrajPlanner_obj->initTraj[0].size() - 1) + 1; i++) {
          math::AABox2d box;
          if (!env->GenerateCorridorBox(0.0, plan_sert[robot_ind][i][0], plan_sert[robot_ind][i][1], 0.5, box)) {
    ...
    timer_step.reset();
    {
        MPCPlanner_obj.reset(new MPCPlanner(constraint_set, corridor_obj, initTrajPlanner_obj, mission, param));
    ...
    writeTrajectoryToYAML_FG(plan_set, 5 * (initTrajPlanner_obj->initTraj[0].size() - 1) + 1, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/ras_demo/formation_generation/fg_complete.yaml");  
    ...
    //   break;
    // }
    // writeTrajectoryToYAML(solution_car_like1, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/traj_car1.yaml");
