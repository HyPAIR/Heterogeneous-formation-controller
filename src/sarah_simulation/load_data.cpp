...
void PlotPose(std::shared_ptr<formation_planner::PlannerConfig> config, int robot_index, int time_step, Trajectory_temp traj) {
...
std::vector<double> GenerateOptimalTimeProfileSegment(const std::vector<double> &stations, double start_time, std::shared_ptr<formation_planner::PlannerConfig> config_) {
...
FullStates ResamplePath(const std::vector<math::Pose> &path, std::shared_ptr<formation_planner::PlannerConfig> config_) {
...
int main(int argc, char* argv[]) {
    double scale = 3;
    double step_size = 0.1;
    std::ifstream file("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/data.json");
    ...
    ros::NodeHandle nh;
    visualization::Init(nh, "map", "/mp_test_vis");
    ros::Publisher path_pub = nh.advertise<nav_msgs::Path>("/mp_test_path", 1, false);\
    ...
    traj_all = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/sarah_traj.yaml");
    readtime_count(time_set, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/sarah_time.yaml");
    ...
    while (ros::ok()) {
      ...
      // }
      // writeTrajectoryToYAML(sol_all, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/sarah_traj.yaml");
      // writetime_count(time_set, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/sarah_time.yaml");
      ros::spinOnce();
      rate.sleep();	
	}
    return 0;
