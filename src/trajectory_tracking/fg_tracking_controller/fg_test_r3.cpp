...
int main(int argc, char **argv)
{
	...
	ros::Duration dt;
	// int robot_index = 2;
	// auto plan =  generate_traj_fg("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/fg_plan.yaml", 10);
	int robot_index = 0;
    ...
    ros::NodeHandle n;
    ros::Subscriber sub_odometry_robot1 = n.subscribe("/mir3/mir_pose_simple", 1000 , robot_actual_pose2);
	...
	while(path_index < plan[robot_index].size() - 2 && ros::ok()){
		...
		if (hypot(robot_pose3.x - plan[robot_index].back()[0], robot_pose3.y - plan[robot_index].back()[1]) < 0.5 && fabs(robot_pose3.theta - plan[robot_index].back()[2]) < 0.1) {
			break;
		}
		distances_old = distances;
