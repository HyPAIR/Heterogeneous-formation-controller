...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n;
    ros::Subscriber sub_odometry_robot1 = n.subscribe("/mir4/mir_pose_simple", 1000 , robot_actual_pose2);
	...
	while(path_index < plan[robot_index].size() - 2 && ros::ok()){
		...
		if (hypot(robot_pose4.x - plan[robot_index].back()[0], robot_pose4.y - plan[robot_index].back()[1]) < 0.5 && fabs(robot_pose4.theta - plan[robot_index].back()[2]) < 0.1) {
			break;
		}
		distances_old = distances;
