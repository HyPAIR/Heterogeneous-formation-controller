...
int cal_idle_index(math::Pose hp_pose, int inf_cs_lp, int inf_cs_hp, int sup_cs_hp, int current_ts, Trajectory_temp traj_lp,
  Trajectory_temp traj_hp, std::shared_ptr<formation_planner::Environment> env, std::shared_ptr<formation_planner::PlannerConfig> config);
std::vector<int>  refine_replan(const std::vector<int>& inputVector);
bool CheckCanstop(Trajectory_temp traj, int stop_pt, std::shared_ptr<formation_planner::PlannerConfig> config);
std::vector<int> CheckSECollision(const std::vector<int>& arr1, const std::vector<int>& arr2);
bool CheckTrajIntersect(const std::vector<Trajectory_temp>& traj_set, std::shared_ptr<formation_planner::PlannerConfig> config);
int countNumInterset(const std::vector<int>& inputVector);
int CalPairCSbegin(Trajectory_temp traj_1, Trajectory_temp traj_2, std::shared_ptr<formation_planner::PlannerConfig> config);
...
void DrawTrajectoryRviz(const FullStates sol_traj, std::shared_ptr<formation_planner::PlannerConfig> config,
  ...
  for (int i = 0; i < sol_traj.states.size(); i++) {
    auto x0_disc = config->vehicle.GetVertexPositions(sol_traj.states[i].x, sol_traj.states[i].y, sol_traj.states[i].theta, 1.7);
  ...
  for (int i = 0; i < traj_set.size(); i++) {
    for (int j = i + 1; j < traj_set.size(); j++) {
      ...
      for (int k = 0; k < traj_length; k++) {
        auto x0_disc = config->vehicle.GetVertexPositions(temp_i[k].x, temp_i[k].y, temp_i[k].theta, 1.7);
        auto x1_disc = config->vehicle.GetVertexPositions(temp_j[k].x, temp_j[k].y, temp_j[k].theta, 1.7);
...
int CalPairCSbegin(Trajectory_temp traj_1, Trajectory_temp traj_2, std::shared_ptr<formation_planner::PlannerConfig> config) {
    ...
    for (int i = 0; i < traj_2.size(); i++) {
      auto x0_disc = config->vehicle.GetVertexPositions(traj_2[i].x, traj_2[i].y, traj_2[i].theta, 1.7);
    ...
    for (int i = 0; i < traj_1.size(); i++) {
      auto x0_disc = config->vehicle.GetVertexPositions(traj_1[i].x, traj_1[i].y, traj_1[i].theta, 1.7);
      std::vector<VertexPt> points = {{x0_disc[6], x0_disc[7]}, {x0_disc[4], x0_disc[5]}, {x0_disc[2], x0_disc[3]}, {x0_disc[0], x0_disc[1]}, {x0_disc[6], x0_disc[7]}};
      boost::geometry::model::polygon<VertexPt> polygon;
      boost::geometry::assign_points(polygon, points); 
...
void RemoveRedundentCS(const int traj_1_index, const int traj_2_index, Trajectory_temp& traj_1, Trajectory_temp& traj_2, 
  const std::vector<std::vector<std::set<math::Pose>>>& critical_section_sets, std::shared_ptr<formation_planner::PlannerConfig> config) {
...
std::vector<std::vector<Trajectory_temp>> UpdatePairTraj(std::shared_ptr<formation_planner::fpLocalPlanner> planner, 
  std::shared_ptr<formation_planner::Environment> env, std::shared_ptr<formation_planner::PlannerConfig> config, int traj_1_index, int traj_2_index,
    std::vector<formation_planner::TrajectoryPoint_temp> traj_1, std::vector<formation_planner::TrajectoryPoint_temp> traj_2, 
    ...
    auto x0_disc = config->vehicle.GetVertexPositions(traj_1[critical_section12[0]].x, traj_1[critical_section12[0]].y, traj_1[critical_section12[0]].theta, 1.7);
    std::vector<VertexPt> points1 = {{x0_disc[6], x0_disc[7]}, {x0_disc[4], x0_disc[5]}, {x0_disc[2], x0_disc[3]}, {x0_disc[0], x0_disc[1]}, {x0_disc[6], x0_disc[7]}};
    ...
    x0_disc = config->vehicle.GetVertexPositions(traj_1[critical_section12[critical_section12.size() - 1]].x, traj_1[critical_section12[critical_section12.size() - 1]].y, traj_1[critical_section12[critical_section12.size() - 1]].theta, 1.7);
    ...
    // 
    std::vector<formation_planner::TrajectoryPoint_temp> replan_traj;
    ...
    // }
    // writeTrajectoryToYAML(replan_result, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/replan.yaml");
    // writeTrajectoryToYAML(solution_diff_drive, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/replan_diff.yaml");
    // writeTrajectoryToYAML(solution_car_like, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/replan_car.yaml");
    // writeTrajectoryToYAML(leader_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/leader_h.yaml");
    // writeTrajectoryToYAML(solution_diff_drive_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/solution_diff_drive_h.yaml");
    // writeTrajectoryToYAML(solution_car_like_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/solution_car_like_h.yaml");

    // writeTrajectoryToYAML(replan_result, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_following/replan.yaml");
    // writeTrajectoryToYAML(solution_diff_drive, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_following/replan_diff.yaml");
    // writeTrajectoryToYAML(solution_car_like, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_following/replan_car.yaml");
    // writeTrajectoryToYAML(leader_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_following/leader_h.yaml");
    // writeTrajectoryToYAML(solution_diff_drive_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_following/solution_diff_drive_h.yaml");
    // writeTrajectoryToYAML(solution_car_like_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_following/solution_car_like_h.yaml");

    // writeTrajectoryToYAML(replan_result, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/replan.yaml");
    // writeTrajectoryToYAML(solution_diff_drive, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/replan_diff.yaml");
    // writeTrajectoryToYAML(solution_car_like, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/replan_car.yaml");
    // writeTrajectoryToYAML(leader_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/leader_h.yaml");
    // writeTrajectoryToYAML(solution_diff_drive_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/solution_diff_drive_h.yaml");
    // writeTrajectoryToYAML(solution_car_like_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/solution_car_like_h.yaml");
    // writeTrajectoryToYAML(replan_result, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/replan.yaml");
    // writeTrajectoryToYAML(solution_diff_drive, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/replan_diff1.yaml");
    // writeTrajectoryToYAML(solution_car_like, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/replan_car1.yaml");
    // writeTrajectoryToYAML(leader_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/replan2.yaml");
    // writeTrajectoryToYAML(solution_diff_drive_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/replan_diff2.yaml");
    // writeTrajectoryToYAML(solution_car_like_h, "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/replan_car2.yaml");
...
void GenerateOptimalTrajectory(std::vector<std::vector<Trajectory_temp>>& traj_set,
  ...
  std::shared_ptr<formation_planner::FormationPlanner> planner) {
...
int cal_idle_index(math::Pose hp_pose, int inf_cs_lp, int inf_cs_hp, int sup_cs_hp, int current_ts, Trajectory_temp traj_lp,
  ...
  for (int i = current_ts; i < traj_hp.size(); i++) {
    auto x0_disc = config->vehicle.GetVertexPositions(traj_hp[i].x, traj_hp[i].y, traj_hp[i].theta, 1.7);
    std::vector<VertexPt> points = {{x0_disc[6], x0_disc[7]}, {x0_disc[4], x0_disc[5]}, {x0_disc[2], x0_disc[3]}, {x0_disc[0], x0_disc[1]}, {x0_disc[6], x0_disc[7]}};
    boost::geometry::model::polygon<VertexPt> polygon; 
    boost::geometry::assign_points(polygon, points); 
    poly_set.push_back(polygon);
  }
  auto x0_disc = config->vehicle.GetVertexPositions(traj_hp[current_ts].x, traj_hp[current_ts].y, traj_hp[current_ts].theta, 1.7);
  std::vector<VertexPt> points = {{x0_disc[6], x0_disc[7]}, {x0_disc[4], x0_disc[5]}, {x0_disc[2], x0_disc[3]}, {x0_disc[0], x0_disc[1]}, {x0_disc[6], x0_disc[7]}};
  ...
  while (closest_index_lp < traj_lp.size()) {
    bool hp_free = true;
    closest_index_lp += 1;
    auto x0_disc = config->vehicle.GetVertexPositions(traj_lp[closest_index_lp].x, traj_lp[closest_index_lp].y, traj_lp[closest_index_lp].theta, 1.7);
    std::vector<VertexPt> points = {{x0_disc[6], x0_disc[7]}, {x0_disc[4], x0_disc[5]}, {x0_disc[2], x0_disc[3]}, {x0_disc[0], x0_disc[1]}, {x0_disc[6], x0_disc[7]}};
    boost::geometry::model::polygon<VertexPt> polygon; 
    boost::geometry::assign_points(polygon, points); 
  ...
  poly_set.clear();
  x0_disc = config->vehicle.GetVertexPositions(traj_lp[closest_index_lp].x, traj_lp[closest_index_lp].y, traj_lp[closest_index_lp].theta, 1.7);
...
int main(int argc, char **argv) {
  ...
  ros::Publisher path_pub_car2 = nh.advertise<nav_msgs::Path>("/fp_test_path_car2", 1, false);
  ros::Publisher path_pub_car3 = nh.advertise<nav_msgs::Path>("/fp_test_path_car3", 1, false);
  ros::Publisher path_pub_car4 = nh.advertise<nav_msgs::Path>("/fp_test_path_car4", 1, false);
  ros::Publisher path_pub_car5 = nh.advertise<nav_msgs::Path>("/fp_test_path_car5", 1, false);
  ros::Publisher path_pub_car6 = nh.advertise<nav_msgs::Path>("/fp_test_path_car6", 1, false);
...
//   });
  ...
  // auto traj_1 = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_following/follow_1.yaml")
  // auto traj_2 = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_following/follow_2.yaml");
  // auto traj_1 = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/opposit_1.yaml");
  // auto traj_2 = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/opposit_2.yaml");
  // auto traj_1 = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/trl_3.yaml");
  // auto traj_2 = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/trl_2.yaml");
  // auto traj_3 = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/three_robots_linear/trl_1.yaml");
  traj_set.resize(num_formation, std::vector<Trajectory_temp>(3));
  ...
  // std::string file_front = "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/five_robtos_goal_oppositting/";
  // std::string file_front = "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_following/";
  std::string file_front = "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/ras_demo/formation_coordination/H_environment/formation1/";
  ...
  auto traj_2 = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/trl_2.yaml");
  ...
  // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  auto traj_1_ori = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/ras_demo/formation_coordination/H_environment/formation1/trl_1.yaml");
  auto sol_1_ori = traj2fullstates(traj_1_ori);
  auto traj_2_ori = generate_traj("/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/two_robots_oppositing/trl_2.yaml");
  ...
  std::string robot_type;
  for (int i = 0; i < traj_set.size(); i++) { 
    file_front = "/home/weijian/Heterogeneous_formation/src/formation_planner/traj_result/ras_demo/formation_coordination/";
    robot_type = "leader";
  ...
  while(ros::ok()) { 
    ...
    for (int j = 0; j < solution_set.size(); j++) {
      DrawTrajectoryRviz(traj2fullstates(traj_set[j][0]), config_, j*3+0, path_pub_set[j*2+0]);
      DrawTrajectoryRviz(traj2fullstates(traj_set[j][0]), config_, j*3+1, path_pub_set[j*2+1]);
      // DrawTrajectoryRviz(traj2fullstates(traj_set[j][2]), config_, j*3+2, path_pub_set[j*2+2]);
    }
    ros::spinOnce();
    r.sleep();
  }
